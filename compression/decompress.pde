...
String decompb(byte[] in) {
  ...
  while(pos<in.length) {
    ...
    if (eq.equals("000") | eq.equals("111")) {
      ...
      lastD = length;
      print ("custom dictionary "+(eq.equals("111")?"long ":"")+"string with", charAm, "characters: \"");
      ...
      for (byte c : chars) {
        ...
        print(compChars.charAt(c)+(i==chars.length-1?"\", and length "+length+": \"":"\", \""));
        i++;
      }
    ...
    if (eq.equals("011")) {
      ...
      println (length,"characters: \""+tout+"\"");
      out+= tout;
    }
    last = eq;
  }
